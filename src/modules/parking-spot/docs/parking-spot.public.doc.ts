import { HttpStatus, applyDecorators } from '@nestjs/common';
import { ENUM_DOC_REQUEST_BODY_TYPE } from 'src/core/doc/constants/doc.enum.constant';
import {
    Doc,
    DocRequest,
    DocResponse,
} from 'src/core/doc/decorators/doc.decorator';
import { ParkingSpotDocParamsId } from '../constants/parking-spot.doc.constant';

// export function ParkingSpotPublicParkVehicleDoc(): MethodDecorator {
//     return applyDecorators(
//         Doc({ operation: 'modules.public.parkingSpots' }),
//         DocRequest({ bodyType: ENUM_DOC_REQUEST_BODY_TYPE.JSON }),
//         DocResponse('user.signUp', { httpStatus: HttpStatus.CREATED })
//     );
// }

export function ParkingSpotPublicParkVehicleDoc(): MethodDecorator {
    return applyDecorators(
        Doc({ operation: 'modules.admin.parkingSpots' }),
        DocRequest({ params: ParkingSpotDocParamsId }),
        DocResponse('parkingSpot.parkVehicle', {
            httpStatus: HttpStatus.CREATED,
        })
    );
}
